...
async function run() {
  try {
    ...
    app.post("/send-money", async (req, res) => {
      ...
      // Validate Donar
      const validateDonar = await usersCollection.findOne({
        mobile: transactionData.donar,
      });
      const isMatch = await bcrypt.compare(
        transactionData.pin.toString(),
        validateDonar.pin
      );
      if (!isMatch) {
        return res.send({
          message: "Your pin is not Correct",
          insertedId: null,
        });
      }

      // Validate Recipient
      const validateRecipient = await usersCollection.findOne({
        mobile: transactionData.recipient,
      });
    ...
    app.post("/cash-out", verifyToken, async (req, res) => {
      ...
      if (validateRecipient.role !== "agent") {
        return res.send({
          message: "Recipient is not agent",
          insertedId: null,
        });
      }
    ...
    app.get("/my-transactions/:mobile", verifyToken, async (req, res) => {
      ...
      const result = await transactionsCollection
        .find({ $or: [{ donar: mobile }, { recipient: mobile }] })
        .toArray();
      res.send(result);
    });
    ...
    //Request related api
    app.get("/my-transactions-req/:mobile", verifyToken, async (req, res) => {
      const mobile = req.params.mobile;
      const query = [
        {
          donar: mobile,
      ...
      const result = await requestCollection.find({ $or: query }).toArray();
      res.send(result);
    });
    ...
    app.post("/accept-req/:id", verifyToken, verifyAgent, async (req, res) => {
      ...
      if (reqData.process === "Cash In Request") {
        ...
        reqProcess = "Cash Out";
        const vat = reqData.amount * 0.015;
        donarAmount = Donar.balance - (reqData.amount + vat);
        recipientAmount = Recipient.balance + (reqData.amount + vat);
      }
